...
class EnergyOptimizerDevice extends RCTDevice {
  ...
  async startOptimizer() {
    ...
    try {
      ...
      this.log('Ô£ô Energy Optimizer started successfully');
  ...
  async updateDeviceData() {
    ...
    if (this._updateDeviceDataRunning) {
      this.debug('ÔÅ¡´©Å Skipping update tick (previous still running)');
      return;
    }
    ...
    try {
      ...
      if (currentHour === fetchHour && currentMinute === 0 && this.lastDailyFetchDate !== currentDate) {
        this.log(`­ƒöä Daily price fetch triggered at ${now.toLocaleTimeString('de-DE')}`);
      ...
      if (isQuarterHour && this.lastDataCollectionMinute !== currentMinute) {
        this.log(`­ƒôè Data collection triggered at ${now.toLocaleTimeString('de-DE')}`);
      ...
      if (isQuarterHour && this.lastOptimizationCheckMinute !== currentMinute) {
        this.log(`ÔÜí Optimization check triggered at ${now.toLocaleTimeString('de-DE')}`);
  ...
  async fetchPricesFromTibber() {
    try {
      ...
      if (this._tibberCircuit.nextAllowedAt && now < this._tibberCircuit.nextAllowedAt) {
        ...
        this.debug(`ÔÅ│ Tibber fetch blocked by circuit breaker (${waitSec}s remaining)`);
        return false;
      }

      this.log('­ƒôí Fetching Tibber prices from API...');
      ...
      this.log(`Ô£à Fetched ${this.priceCache.length} price intervals (from ${this.priceCache.length > 0 ? new Date(this.priceCache[0].startsAt).toLocaleString() : 'N/A'})`);
  ...
  async calculateOptimalStrategy({ force = false } = {}) {
    ...
    if (!force && this._lastStrategyInputHash) {
      ...
      if (currentHash === this._lastStrategyInputHash) {
        this.debug('ÔÅ¡´©Å Skipping strategy recalc (no input changes)');
        return;
      }
    ...
    this.log('\n­ƒöä Recalculating optimization strategy with current battery status...');

    if (!this.priceCache.length) {
      this.log('ÔÜá´©Å No price data available - cannot optimize');
      return;
    }
    ...
    this.log(`Average price: ${avgPrice.toFixed(4)} Ôé¼/kWh`);
    ...
    if (lpResult) {
      ...
      if (nextChargeInterval) {
        ...
        this.log(`Estimated savings: Ôé¼${savings.toFixed(2)}`);
        this.log('======================\n');
      } else {
  ...
  async executeOptimizationStrategy() {
    this.log('\n­ƒöä === EXECUTING OPTIMIZATION STRATEGY ===');
    this.log(`Timestamp: ${new Date().toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })}`);

    if (!this.getCapabilityValue('onoff')) {
      this.log('ÔÜá´©Å Optimizer is disabled, skipping execution');
      return;
    }

    if (!this.currentStrategy) {
      this.log('ÔÜá´©Å No strategy available, skipping execution');
      return;
    }
    ...
    try {
      ...
      this.log(`ÔÜá´©Å Could not update next_charge_start: ${error.message}`);
    }
    ...
    if (decision.mode === BATTERY_MODE.IDLE) {
      this.log('ÔÜá´©Å No action to take');
      return;
    }
  ...
  async collectGridPower() {
    ...
    try {
      ...
      this.log('   ÔÜá´©Å Grid device found but no measure_power capability');
      return 0;
    } catch (error) {
      this.log(`   ÔÜá´©Å Could not read grid power: ${error.message}`);
      return 0;
    }
  }
  ...
  async applyBatteryMode(batteryDevice, decision) {
    ...
    const priceInfo = intervalIndex >= 0 && this.priceCache[intervalIndex]
      ? `(${this.priceCache[intervalIndex].total.toFixed(4)} Ôé¼/kWh)`
      : '';

    switch (mode) {
      case BATTERY_MODE.CHARGE:
        this.log(`Ô£à CHARGE INTERVAL ACTIVE (index ${intervalIndex})`);
        this.log(`   ÔåÆ ${reason}`);

        if (typeof batteryDevice.enableGridCharging === 'function') {
          await batteryDevice.enableGridCharging();
          this.log('   Ô£ô enableGridCharging() completed successfully');
          ...
          this.error('ÔØî Battery device does not have enableGridCharging method');
        }
        break;

      case BATTERY_MODE.NORMAL:
        this.log(`­ƒÅá NORMAL MODE (index ${intervalIndex})`);
        this.log(`   ÔåÆ ${reason}`);

        if (typeof batteryDevice.enableDefaultOperatingMode === 'function') {
          await batteryDevice.enableDefaultOperatingMode();
          this.log('   Ô£ô enableDefaultOperatingMode() completed successfully');
          ...
          this.error('ÔØî Battery device does not have enableDefaultOperatingMode method');
        }
        break;

      case BATTERY_MODE.CONSTANT:
        this.log(`­ƒöÆ CONSTANT MODE (index ${intervalIndex})`);
        this.log(`   ÔåÆ ${reason}`);

        if (typeof batteryDevice.disableBatteryDischarge === 'function') {
          await batteryDevice.disableBatteryDischarge();
          this.log('   Ô£ô disableBatteryDischarge() completed successfully');
          ...
          this.error('ÔØî Battery device does not have disableBatteryDischarge method');
        }
        break;

      default:
        this.log(`ÔÜá´©Å Unknown mode: ${mode}`);
    }
  }
  ...
  async logBatteryModeChange(mode, details = '') {
    try {
      ...
      this.log(`­ƒôó Timeline notification: ${fullMessage}`);
    } catch (error) {
  ...
  async trackBatteryCharging(batteryPower) {
    try {
      ...
      if (shouldClearChargeLog(currentSoc, minSocThreshold, this.batteryChargeLog.length)) {
        this.log(`­ƒöä Battery at ${currentSoc.toFixed(1)}% (Ôëñ ${minSocThreshold}%) - clearing charge log (${this.batteryChargeLog.length} entries)`);
      ...
      if (!this.lastMeterReading) {
        this.log('­ƒôè Initializing meter readings (first run or after restart)');
        this.lastMeterReading = {
          solar: solarMeterNow,
          grid: gridImportedNow,
          gridImported: gridImportedNow,
      ...
      if (batteryChargedKWh > 0.001) {
        ...
        this.log(`Battery charged: ${batteryChargedKWh.toFixed(3)} kWh (${chargeEntry.solarKWh.toFixed(3)} solar + ${chargeEntry.gridKWh.toFixed(3)} grid @ ${chargePrice} Ôé¼/kWh) [SoC: ${socInfo}%]`);
      ...
      if (batteryDischargedKWh > 0.001) {
        ...
        this.log(`  Avg battery cost before discharge: ${avgBatteryPrice.toFixed(4)} Ôé¼/kWh`);
        this.log(`  Current grid price: ${(currentPrice || 0).toFixed(4)} Ôé¼/kWh`);
      ...
      if (this.batteryChargeLog.length > MAX_BATTERY_LOG_ENTRIES) {
        this.log(`ÔÜá´©Å Battery log reached ${this.batteryChargeLog.length} entries - removing oldest`);
  ...
  calculateBatteryEnergyCost() {
    try {
      this.log('\n­ƒöì calculateBatteryEnergyCost called:');
      ...
      if (result) {
        this.log(`   Ô£à Result: ${result.totalKWh.toFixed(3)} kWh @ ${result.avgPrice.toFixed(4)} Ôé¼/kWh`);
        ...
        this.log('   ÔÜá´©Å Result: null (no data or battery empty)');
      }
  ...
  estimateBatteryEnergyCost(currentSoc, batteryCapacity) {
    try {
      ...
      if (this.currentStrategy?.chargeIntervals && this.currentStrategy.chargeIntervals.length > 0) {
        ...
        this.log(`   Using avg planned charge price as estimate: ${estimatedPrice.toFixed(4)} Ôé¼/kWh`);
      ...
      // Option 2: Use average of recent price data
        ...
        this.log(`   Using avg recent price as estimate: ${estimatedPrice.toFixed(4)} Ôé¼/kWh`);
      } else {
        this.log(`   Using fallback price as estimate: ${estimatedPrice.toFixed(4)} Ôé¼/kWh`);
      }
      ...
      this.log('   ­ƒôè Estimated battery cost (source unknown):');
      this.log(`      Total: ${totalKWh.toFixed(2)} kWh @ ${weightedAvgPrice.toFixed(4)} Ôé¼/kWh (estimated)`);
  ...
  combineBatteryCost(tracked, totalKWh, batteryCapacity) {
    ...
    this.log('   ­ƒôè Combined battery cost (tracked + unknown):');
    this.log(`      Total: ${combinedTotalKWh.toFixed(2)} kWh @ ${combinedAvgPrice.toFixed(4)} Ôé¼/kWh`);
    this.log(`      Tracked: ${trackedKWh.toFixed(2)} kWh @ ${(tracked?.avgPrice || 0).toFixed(4)} Ôé¼/kWh`);
    this.log(`      Unknown: ${unknownKWh.toFixed(2)} kWh @ ${unknownAvgPrice.toFixed(4)} Ôé¼/kWh (estimated)`);
  ...
  async updateBatteryStatus(batteryDevice) {
    ...
    try {
      ...
      this.log(`Ô£à Battery status updated: SoC ${(currentSoc * 100).toFixed(1)}%, energyCost: ${batteryCostInfo ? 'available' : 'null'}`);
    } catch (error) {
